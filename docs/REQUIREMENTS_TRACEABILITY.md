...
| ZBW-DEPLOY-005 | M21 Phase 3 COMPLETE: isolated CloudNet discovery, secret-free metadata, bounded callbacks, fenced scaling, drain and crash replacement | cloud/zbw-cloudnet | cloudnet commands / cloudnet.* remain later M21 presentation | CloudNet lifecycle manager | ServiceDiscovery API plus M19 lease and M20 registry ports | provider health and bounded queue diagnostics | discovery/scaling/drain/crash/degradation CT PASS | `CLOUDNET_INTEGRATION_M21.md`; M21 Phase 3 |
...
| ZBW-INT-002 | M21 Phase 2 isolated Vault economy adapter delegates neutral balance/transaction intents; M12 ledger remains authoritative | integrations/vault | provider lifecycle diagnostics later / manage later | integration manager later | `EconomyProvider` plus optional lifecycle | provider health later | present/absent/incompatible and delegation CT PASS | `PROVIDER_INTEGRATIONS_M21.md`; M21 Phase 2 adapter / later production binding |
| ZBW-INT-003 | M21 Phase 2 isolated LuckPerms permission/meta/context projection with bounded invalidation versions; no profile ownership | integrations/luckperms | provider lifecycle diagnostics later / manage later | integration manager later | `PermissionProvider` plus optional lifecycle | safe rank meta later | context/meta/invalidation CT PASS | `PROVIDER_INTEGRATIONS_M21.md`; M21 Phase 2 adapter / later production binding |
...
| ZBW-INT-006 | M21 Phase 2 Citizens and ZNPCsPlus presentation-only adapters behind isolated gateways; internal NPC ownership unchanged | integrations/citizens,znpcs | NPC management later / npc.* later | NPC manager later | `NpcProvider` plus optional lifecycle | safe NPC state later | present/absent/incompatible/provider CT PASS | `PROVIDER_INTEGRATIONS_M21.md`; M21 Phase 2 adapter / later migration UI |
| ZBW-INT-007 | M21 Phase 2 DecentHolograms presentation-only adapter behind an isolated gateway | integrations/hologram | hologram management later / hologram.* later | hologram manager later | `HologramProvider` plus optional lifecycle | content values later | mapping/provider/absence CT PASS; PT later | `PROVIDER_INTEGRATIONS_M21.md`; M21 Phase 2 adapter / later production qualification |
| ZBW-INT-008 | M21 Phase 2 event-push Grim/Vulcan signal adapters with bounded alert-ID dedupe; M18 remains sole case/verdict owner | integrations/anticheat | alert diagnostics later / anticheat.* later | alert manager later | `AntiCheatProvider` plus optional lifecycle | safe violation values later | normalization/dedupe/duplicate-subscription CT PASS; dual runtime PT/ST later | `PROVIDER_INTEGRATIONS_M21.md`; M21 Phase 2 adapters / later production qualification |
| ZBW-INT-009 | M21 Phase 1 native party aggregate/SQL authority plus Phase 2 migration-only AlessioDP adapter with authorization fence; no live split brain | parties/integrations/party | party + admin later / party.* later | party/admin later | `PartyProvider`, immutable snapshots/intents and optional lifecycle | party fields later | lifecycle/privacy/expiry/SQL recovery plus migration-authority CT PASS | `PARTY_M21_PHASE1.md`; `PROVIDER_INTEGRATIONS_M21.md`; M20 transport / M21 Phase 1-2 |
