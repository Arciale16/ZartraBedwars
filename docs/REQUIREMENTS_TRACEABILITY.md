...
| Requirement | Planned implementation and data/migration | Configuration | Cmd / permission | GUI | API / events | PH | Tests | Documentation / milestone |
|---|---|---|---|---|---|---|---|---|
| ZBW-GAME-001 | M08 Java-8 match/session/team state machine and use cases; closed event/effect translation in `zbw-paper-modern` | modes/arenas/performance | M09 join/start/forcestart adapters / play,force; M08 use cases enforce authorization | M09 waiting/admin arena surfaces | M08 Game/Arena APIs + lifecycle events | M16 arena state/countdown | M08 state/property/primary-Paper E2E; M09 surface E2E | gameplay guide; M08 core/primary Paper / M09 presentation |
| ZBW-GAME-002 | M08 combat/elimination/phase policies and primary Paper event translation; mode-specific policy remains M10 | modes/events/items | M09 game-event controls / force | M09/M10 spectator/admin presentation | M08 combat/phase events | M16 bed/team/time state | M08 core/primary E2E; M10 gameplay matrix; M09 surface E2E | gameplay/events guide; M08 core / M09 presentation / M10 modes |
| ZBW-GAME-003 | M08 completion orchestration, outbox coordination and player-state restore; later reward/stat/replay consumers retain ownership | rewards/restore | M09 end/force adapters | M09/M12 reward/end summary | M08 completion contract/events; M12/M15/M17 consumers | M16 result/rewards | M08 retry/crash/exactly-once/restore E2E; later consumer suites | recovery/reward guide; M08 core/primary Paper / M09 presentation / M12+ consumers |
...
| ZBW-GAME-008 | M08 state-specific hotbar definitions, precedence, action intents and transition policy; `zbw-paper-modern` applies/restores the closed primary hotbar | gui/items | M09 hotbar reload/manage adapters | M09 unified hotbar editor/preview/confirmation | M08 Hotbar API/events | M16 visible hotbar state | M08 transition/leak/primary-Paper E2E; M09 editor/command E2E; M22 compatibility | hotbar guide; M08 core/primary Paper / M09 presentation / M16/M22 completion |
...
| ZBW-GAME-010 | M05 verified recovery substrate; M08 match/player/world recovery steps, safe routing and primary Paper state restoration | scheduler deadlines/lifecycle budgets; feature recovery config M08 | M09 recover/health/admin.force adapters; M08 use cases enforce authorization | M09 recovery GUI | M08 recovery use cases/events over verified M05 marker/store/step/report contracts | M16 typed recovery state | M05 substrate VERIFIED; M08 ordered/runtime/crash/restore E2E; M09 surface E2E | `LIFECYCLE_M05.md`; M05 foundation VERIFIED / M08 runtime / M09 presentation |
...
| ZBW-ARENA-002 | M07 immutable map aggregate, mutable presentation snapshot and rename/reference policy | maps; M07 binding/migration | M09 map edit/rename/inspect / map.* | M09 map manager | M07 Map API/events | M16 map fields | M07 rename/reference UT; M09 surface E2E | map guide; M07 core / M09 presentation |
...
| ZBW-ARENA-008 | M07 validation rule registry, actionable report and block-enable policy | all arena/world configs | M09 setup/arena validate / validate | M09 validator GUI | M07 Validation API/events | M16 validation state | M07 mutation/rule/enable-gate UT/E2E; M09 surface E2E | validation guide; M07 core / M09 presentation |
...
| Requirement | Planned implementation and data/migration | Configuration | Cmd / permission | GUI | API / events | PH | Tests | Documentation / milestone |
|---|---|---|---|---|---|---|---|---|
| ZBW-DEPLOY-001 | M08 shared-server game/session/arena assembly and bounded primary Paper runtime foundation; optional providers and release qualification remain later | deployment/performance | M09 deployment/health adapters / admin | M09 health/world/arena presentation; M21 provider panels | M08 Deployment API/events foundation | M16 deployment counts | M08 primary shared-server E2E; M21 providers; M24 40-world/10-active PT | shared-server guide; M08 foundation / M09 presentation / M21/M24 completion |
...
| M07/M09 / ZBW-ADDON-411 | Team create/edit and team-spawn policies/use cases | Team setup commands and editor pages | M16 placeholders; M22 compatibility |
...
| M07/M09 / ZBW-ADDON-419 | Arena mode/group/team/bounds/world-adapter policies, validation and draft mutation | Property commands and arena-property editor | M16 placeholders; M21 provider adapters; M22 compatibility |
