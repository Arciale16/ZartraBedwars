...
| Requirement | Planned implementation and data/migration | Configuration | Cmd / permission | GUI | API / events | PH | Tests | Documentation / milestone |
|---|---|---|---|---|---|---|---|---|
| ZBW-ARC-001 | root `pom.xml`; `build/zbw-bom`; `build/zbw-build-tools`; planned boundaries in `build/module-graph.json` | Maven/module manifests | M1 validator / maintainer | — (no runtime module health in M01) | no lifecycle API until M02 | — | exact reactor/cycle/forbidden-dependency checks | architecture + implementation guide; M01 |
...
| ZBW-ARC-008 | M1 machine schemas explicitly versioned; runtime schema registry/migrations remain M04 | `schema_version` in every M1 JSON manifest | M1 validator / maintainer; runtime migrate later | — in M01 | build-report schemas; runtime APIs M04 | — | schema/version and deterministic generation; rolling CT M04 | implementation/migration architecture; M01/M04 |
...
| ZBW-OPS-008 | checksum-locked Maven 3.9.11 wrapper, parent/BOM/build descriptor, two CI workflows, generated lock/SBOM/notices | `.mvn/`, `pom.xml`, `build/*.json`, workflows | `clean_build.py`, `run_m01_validation.py` / maintainer | — | no published API artifact until M02 | build/toolchain versions in manifests | clean five-JDK/offline/reproducible governance CI | implementation guide; M01/M24 |
...
| ZBW-QA-005 | M1 static/module/binary/dependency inventories and default-deny vulnerability surface; feature/API/release gates remain M24 | `build/quality-policy.json`, CI workflows | M1 validator / maintainer | — | API compatibility activates M02+ | — in M01 | zero unapproved binary/dynamic dependency and static-policy gates | quality gates + implementation guide; M01/M24 |
...
| Requirement | Planned implementation and data/migration | Configuration | Cmd / permission | GUI | API / events | PH | Tests | Documentation / milestone |
|---|---|---|---|---|---|---|---|---|
| ZBW-CONTENT-001 | `build/asset-provenance.json` plus production-resource and forbidden-binary scan | exact provenance/rights manifest | M1 validator / maintainer | — (file report in M01) | content API remains M02+ | — in M01 | clean-room asset/hash/repository scan | asset provenance + implementation guide; M01/M24 |
...
| Requirement | Planned implementation and data/migration | Configuration | Cmd / permission | GUI | API / events | PH | Tests | Documentation / milestone |
|---|---|---|---|---|---|---|---|---|
| ZBW-LICENSE-001 | 14-artifact exact build/CI registry, CycloneDX `build/sbom.cdx.json` | acquisition/lock policy JSON | dependency `validate` / maintainer | — (machine/human files) | CycloneDX/lock schema | — | inventory/rights/licence hash scan | dependency audit + implementation guide; M01/M24 |
| ZBW-LICENSE-002 | `dependency-acquisition.json` → generated immutable `dependency-lock.json`; wrapper/JDK archive hash checks | default-deny lock/update policy | lock `generate|validate` / maintainer | — | dependency metadata files | build versions only | SHA-256/SHA-512/git commit/dynamic-version drift CI | dependency audit; M01 |
| ZBW-LICENSE-003 | explicit product redistribution booleans; all M1 tools classified build/CI-only | rights records per artifact | dependency `validate` / maintainer | — | governance records only | — | redistribution/product-binary rejection | licensing guide + notices; M01/M24 |
| ZBW-LICENSE-004 | per-artifact shading/modification/attribution fields; generated build notices | rights/obligation records | dependency `validate` / maintainer | — | SBOM properties | — | missing-rights and product-packaging scan | dependency audit/build notices; M01/M24 |
| ZBW-LICENSE-005 | BOM contains only approved exact selections; zero direct/runtime dependency is resolved in M1 | BOM + scope policy | dependency `validate` / maintainer | — | provider SPIs remain M02/M21 | — | direct dependency and executable-plugin default-deny checks | dependency/integration guides; M01/M21 |
...
| ZBW-LICENSE-007 | deterministic CycloneDX SBOM, build notices and asset provenance cross-gates | notice/provenance policy | M1 validator / maintainer | — (file reports) | notice/SBOM schemas | approved/blocked counts in manifests | regeneration/staleness/licence CI | root/build notices + implementation guide; M01/M24 |
...
| ZBW-READY-005 | 14 verified build/CI artifacts; generated lock/SBOM/notices; Maven dependency resolution remains empty/offline | acquisition/lock policy | dependency `generate|validate` / maintainer | — (reports are files) | CycloneDX/lock schema | build versions only | archive/git/licence/drift/repro gates | dependency audit + ADR-0007 + implementation guide; M01 |
...
| ZBW-READY-008 | exact BOM/plugin properties, manual-DI/native-framework policy and structural checks; no dependency resolved before lock | parent/BOM/quality/module manifests | M1 validator / maintainer | — | command/GUI/provider SPIs remain M02+ | — | BOM/version/architecture/licence tests | dependency audit + ADR-0007 + implementation guide; M01 |
...
| ZBW-READY-017 | two module-aware CI workflows; exact quality manifest; static/bytecode/dependency/docs gates and tests | `quality-policy.json`, pinned workflows | M1 validator / maintainer | — | machine-readable quality/lock/SBOM reports | — | 8 tests, five-JDK clean build, zero static/dependency/doc regressions; mutation activates with critical Java | quality gates + ADR-0009 + implementation guide; M01/M24 |
