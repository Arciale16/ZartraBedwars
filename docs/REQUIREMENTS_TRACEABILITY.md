...
| ZBW-ARC-002 | platform-free core/version-family boundaries fixed in `build/module-graph.json`; M06 plans Java-8 `zbw-compat-api`/`zbw-world` and Java-21 primary adapter/bootstrap; legacy/intermediate artifacts remain M22 | toolchain/module manifests | graph validator / maintainer | — | semantic compatibility/world contracts in M06 | — | forbidden-layer, bytecode and milestone-order checks; CT M06/M22 | runtime/compatibility guides; M01/M06/M22 |
...
| ZBW-GAME-010 | M05 compare-and-set recovery markers, bounded coordinator, ordered step port, idempotency fence and MANUAL_REQUIRED degradation; match/player/world steps remain M08 | scheduler deadlines/lifecycle budgets; feature recovery config remains M08 | recover/health/admin.force remain M08/M09 | recovery GUI remains M08/M09 | `Recovery` marker/store/step/report/event API | typed recovery state available to later placeholders | ordered success, duplicate fence, step/marker conflict, invalid/incomplete plan and sink-failure tests VERIFIED | `LIFECYCLE_M05.md`; M05 foundation VERIFIED / M08 runtime |
...
| ZBW-ARENA-005 | M06 Java-8 world SPI/orchestration plus native Paper 1.21.1 provider; optional WorldEdit/FAWE/WorldGuard/Slime/Multiverse adapters remain M21 | integrations/world | world load/clone/backup… / world.* later | world manager later | World API/provider events | world/provider health | reusable provider CT; primary E2E M06; optional-provider CT M21 | world provider guide; M06 foundation/M21 completion |
| ZBW-ARENA-006 | M06 bounded native-primary reset pipeline foundation; arena-owned cleanup/runtime integration remains M07 | performance/world | world reset / reset,force later | reset progress later | Reset API/events | reset state/time | owner-thread/I/O split M06; concurrent arena reset PT M07 | reset/tuning guide; M06 foundation/M07 completion |
...
These rows define planning ownership only. M06 is not started by this allocation change and may not mark a continuing requirement complete when its M22 or later acceptance remains open.

| Requirement | M06 foundation | Tests and evidence assigned to M06 | Remaining owner |
|---|---|---|---|
| M06 / ZBW-ARC-002/007 | Java-8 compatibility and world-provider contracts; Java-21 primary adapter/bootstrap dependency boundary | graph/order/cycle, bytecode and forbidden-import checks | Provider families and final architecture evidence in owning milestones/M24 |
| M06 / ZBW-ARENA-005/006 | neutral world SPI/reset orchestration and native Paper 1.21.1 provider | primary load/clone/reset/unload E2E; owner-thread/filesystem split; reusable provider CT | Arena integration M07; optional provider adapters M21 |
| M06 / ZBW-INT-004/005 | primary-safe packet capability boundary and native world provider only | primary capability and provider contracts | ProtocolLib/full packet matrix M22; optional world providers M21 |
| M06 / ZBW-INT-010 | neutral compatibility API and `zbw-compat-v1_20-v1_21` behavior required only by Paper 1.21.1 foundation certification | primary adapter/bootstrap/capability CT | Legacy/intermediate adapters, remaining runtime rows, Via and Bedrock M22 |
| M06 / ZBW-COMPAT-001..009 | semantic capability/fallback contracts, primary mappings, typed degradation/last-known-good policy and primary-row report schema | Paper 1.21.1 foundation, mapping completeness, unsupported/fallback and no-platform-leak tests | `zbw-compat-v1_8`, all legacy fallback implementations and complete feature matrix M22 |
| M06 / ZBW-CONTENT-009 | semantic sound/visual-effect identifiers and primary mappings | primary mapping/config validation CT | Starter catalogue M14; mandatory 1.8 mapping/usability M22 |
| M06 / ZBW-READY-001/002/006 | materialize only the approved Java-8 neutral and Java-21 primary artifacts and consume only the locked Paper 1.21.1 build 133 fixture | class-major, exact-fixture, primary bootstrap/world and no-overclaim evidence | All other runtime artifacts and full release certification M22 |
| M06 / ZBW-READY-003 | compatibility-renderer contract foundation only; no cosmetic definition or runtime effect is delivered | semantic renderer capability CT | 300 definitions/effects M14; legacy rendering M22; release quality M24 |
