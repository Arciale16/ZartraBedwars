...
| ZBW-ARC-002 | VERIFIED M06 Java-8 `zbw-compat-api`/`zbw-world` and Java-21 primary adapter/bootstrap boundaries in `build/module-graph.json`; legacy/intermediate artifacts remain M22 | toolchain/module manifests | graph validator / maintainer | — | semantic compatibility/world contracts | — | exact class-major 52/65, forbidden-layer, graph/order and API CT pass | runtime/compatibility guides; M06 VERIFIED / M22 completion |
...
| ZBW-ARENA-005 | M06 Java-8 world SPI/orchestration plus native Paper 1.21.1 provider; M07 arena lifecycle consumes only that port; M21 isolated WorldEdit/FAWE/WorldGuard/SlimeWorldManager/Multiverse adapters delegate through operator gateways with native fallback | integrations/world/zbw-integration-world-providers | M09 world command adapters / world.* | M09 world manager | World API/provider events | M16 world/provider health | reusable provider CT; primary E2E M06/M07; optional lifecycle/fallback/classloading CT M21 | `WORLD_PROVIDER_M06.md`, `WORLD_PROVIDERS_M21.md`; M06 foundation / M07 core / M09 presentation / M21 adapter completion |
...
M16 is complete with checkpoints `965291c`, `9758c71`, `51af49b`, `0cee0b3`, `badd504` and `586797f`. M17 replay and every later milestone boundary remain unclaimed; M15 remains complete in the same checkpoints.
...
| ZBW-DEPLOY-005 | M21 Phase 3-4: isolated CloudNet discovery, secret-free metadata, bounded callbacks, single-flight fenced scaling, drain, restart and crash replacement | cloud/zbw-cloudnet | command/dashboard presentation remains incomplete | CloudNet lifecycle manager | ServiceDiscovery API plus M19 lease and M20 registry ports | Plugin Doctor health and bounded queue diagnostics | discovery/scaling/drain/crash/restart/stale/race/degradation CT PASS | `CLOUDNET_INTEGRATION_M21.md`; `RUNBOOK_M21_PROVIDERS_CLOUDNET.md` |
...
| ZBW-INT-002 | M21 Phase 2 isolated Vault economy adapter delegates neutral balance/transaction intents; M12 ledger remains authoritative | integrations/vault | Plugin Doctor compatibility diagnostics / manage later | integration manager later | `EconomyProvider` plus optional lifecycle | Plugin Doctor provider state | present/absent/incompatible/duplicate and delegation CT PASS | `PROVIDER_INTEGRATIONS_M21.md`; `RUNBOOK_M21_PROVIDERS_CLOUDNET.md`; M21 Phase 2/4 |
| ZBW-INT-003 | M21 Phase 2 isolated LuckPerms permission/meta/context projection with bounded invalidation versions; no profile ownership | integrations/luckperms | Plugin Doctor compatibility diagnostics / manage later | integration manager later | `PermissionProvider` plus optional lifecycle | safe rank meta later | context/meta/invalidation CT PASS | `PROVIDER_INTEGRATIONS_M21.md`; `RUNBOOK_M21_PROVIDERS_CLOUDNET.md`; M21 Phase 2/4 |
...
| ZBW-INT-005 | M06 WorldProvider SPI/native primary adapter plus M21 isolated WorldEdit 7.3.16, FAWE 2.15.1, WorldGuard 7.0.17, SlimeWorldManager 5.1.0 and Multiverse 5.3.3 adapters; asynchronous exact-version probing, duplicate rejection, classloading isolation and native fallback | integrations/world/zbw-integration-world-providers | operator composition through generic M21 provider runtime; Paper softdepend | Plugin Doctor/provider diagnostics | existing WorldProvider and Provider SPIs | provider compatibility status | adapter unit/contract tests; Paper provider-runtime lifecycle tests; strict Java 8/API/quality gates | `WORLD_PROVIDERS_M21.md`; M21 provider integration complete, M22 full matrix retained |
| ZBW-INT-006 | M21 Phase 2 Citizens and ZNPCsPlus presentation-only adapters behind isolated gateways; internal NPC ownership unchanged | integrations/citizens,znpcs | NPC management later / npc.* later | NPC manager later | `NpcProvider` plus optional lifecycle | safe NPC state later | present/absent/incompatible/duplicate/provider CT PASS | `PROVIDER_INTEGRATIONS_M21.md`; `RUNBOOK_M21_PROVIDERS_CLOUDNET.md`; M21 Phase 2/4 |
| ZBW-INT-007 | M21 Phase 2 DecentHolograms presentation-only adapter behind an isolated gateway | integrations/hologram | hologram management later / hologram.* later | hologram manager later | `HologramProvider` plus optional lifecycle | content values later | mapping/provider/absence CT PASS; PT later | `PROVIDER_INTEGRATIONS_M21.md`; `RUNBOOK_M21_PROVIDERS_CLOUDNET.md`; M21 Phase 2/4 |
| ZBW-INT-008 | M21 Phase 2 event-push Grim/Vulcan signal adapters with bounded alert-ID dedupe; M18 remains sole case/verdict owner | integrations/anticheat | alert diagnostics later / anticheat.* later | alert manager later | `AntiCheatProvider` plus optional lifecycle | safe violation values later | normalization/dedupe/duplicate-subscription plus Grim-only/Vulcan-only/dual CT PASS | `PROVIDER_INTEGRATIONS_M21.md`; `RUNBOOK_M21_PROVIDERS_CLOUDNET.md`; M21 Phase 2/4 |
| ZBW-INT-009 | M21 Phase 1 native party aggregate/SQL authority plus Phase 2 migration-only AlessioDP adapter with authorization fence; no live split brain | parties/integrations/party | party + admin later / party.* later | party/admin later | `PartyProvider`, immutable snapshots/intents and optional lifecycle | party fields later | lifecycle/privacy/expiry/SQL recovery plus fenced migration/no-split-brain CT PASS | `PARTY_M21_PHASE1.md`; `PROVIDER_INTEGRATIONS_M21.md`; `RUNBOOK_M21_PROVIDERS_CLOUDNET.md`; M21 Phase 1-4 |
| ZBW-INT-010 | M06 neutral capability contracts and Paper 1.21.1 primary modern adapter only; M22 completes legacy/intermediate adapters, all remaining runtime rows, Via and Bedrock alternatives | integrations/compatibility | compat diagnostics / debug later | compatibility/alt controls later | Compatibility API/events foundation M06 | server/client version later | primary-foundation CT M06; complete 1.8–1.21/translated/Bedrock matrix M22 | compatibility guide; M06 foundation/M22 completion |
...
| Requirement | Planned implementation and data/migration | Configuration | Cmd / permission | GUI | API / events | PH | Tests | Documentation / milestone |
|---|---|---|---|---|---|---|---|---|
| ZBW-QA-001 | Python governance plus verified Java suites; M08.1 adds deterministic shared-limit, profile, assembler, victory and layout tests | quality/toolchain manifests | governance runner | — | test-only helpers | — | M08.1 zero-skip neutral/Paper regression and quality gates | `IMPLEMENTATION_M08_1.md`; all |
| ZBW-QA-002 | M08.1 VERIFIED foundational 8x1, 8x2, 4x3, 4x4, 2x4, 12x3 and 64x4 lifecycle/layout matrix; full feature/mode E2E remains later | deterministic arena fixtures | — | tested GUIs M09+ | M08.1 typed APIs; later events | tested mode PH M16 | layout foundation M08.1; full gameplay matrix M10/M24 | `LAYOUT_COMPATIBILITY_M08_1.md`; M08.1 foundation / M10/M24 completion |
...
| ZBW-READY-006 | exact public fixture hashes plus explicit private legacy lock states; M06 consumes only the Paper 1.21.1 build 133 fixture and M22 consumes every full row | `private-runtime-fixtures.json` | validator / maintainer; runtime verify later | — before runtime surfaces | fixture report schema/API M06 | no support placeholder before certification | hash/registry/no-binary; primary fixture CT M06; boot/game CT all rows M22 | runtime matrix; M01/M06 foundation/M22 completion |
| ZBW-READY-007 | M02 vendor-neutral event-push contracts plus M21 Phase 2 isolated Grim/Vulcan normalization, bounded dedupe and no-provider lifecycle; M18 remains case/verdict owner | no vendor secret or binary in neutral modules | diagnostics later | health GUI later | neutral provider, alert and optional lifecycle contracts | sanitized health later | M02 contract tests plus M21 normalization/dedupe/absence CT PASS; production dual-provider PT/ST later | `API_M02.md`; `PROVIDER_INTEGRATIONS_M21.md`; M02 SPI / M21 Phase 2 adapter |
...
| ZBW-READY-011 | M04 retention/hold/release/tombstone schema and repository; scheduler/encrypted feature stores remain M17/M18 | retention classes | privacy commands remain M09/M17 | workflow remains M17/M18 | `RetentionPolicy`/`RetentionRepository` | none — private | duplicate/transaction/privacy storage IT plus certified MySQL/MariaDB restart persistence; clock/delete workflow later | privacy policy + M04 API; M04 VERIFIED / M17/M18 |
...
## M20 Phase 1 proxy API foundation

| Allocation | Evidence | Remaining owner |
|---|---|---|
| M20 / ZBW-DEPLOY-002..004 | `zbw-proxy-api` immutable backend identity/epoch/capability, registry/status, heartbeat/capacity/health, deterministic routing, bounded reservation, transfer-token claim/outcome, protocol/degradation and sanitized diagnostics contracts; Java 8 immutable/invalid/stale/expiry/duplicate/determinism/privacy tests | M20 later phases own Velocity/Bungee adapters, backend runtime registry, atomic token store, cross-server flows and provider-equivalence E2E |
| M20 / ZBW-READY-013/014 | Opaque lower-case references, exact audience, 15-second reservation/token bounds, stale epoch rejection and diagnostics without endpoints, secrets or raw player data | M04/M19 retain envelope, HMAC, nonce, dedupe, lease/fencing and SQL-authority implementation; M20 adapters must compose them |
| M20 / ZBW-ADDON-041..060, 102..107, 164..173, 252..259, 291..299, 464..473 | Neutral routing/reservation/transfer contracts are available for later feature adapters without importing game, progression, statistics, replay, Atlas, Redis or platform implementations | Feature behavior, commands/GUI/Paper projection and distributed E2E remain later M20 phases; M11-M19 ownership is unchanged |
## M20 Phase 2 routing and proxy adapters

| Allocation | Materialized evidence | Remaining owner |
|---|---|---|
| M20 / ZBW-DEPLOY-002..004 | Java 8 `BackendRegistry`, deterministic capacity-aware `ProxyRoutingEngine`, bounded `ProxyReservationCoordinator`, and equivalent `BungeeProxyAdapter`/`VelocityProxyAdapter` lifecycle and nonblocking transport boundaries; tests cover stale epochs, heartbeats, drain/unhealthy/offline state, fallback, conflicts, duplicate admission and single-use transfer | Cross-server matchmaking/party/rejoin/play-again flows remain later M20 phases; M10 retains matchmaking policy |
| M20 / ZBW-READY-013/014 | `SignedProxyMessage` and `ProxyMessageSecurity` enforce 64 KiB bounds, 100/s sustained and burst-200 peer limits, protocol/environment/audience/key ID/deadline/nonce checks and HMAC-SHA-256 before payload exposure; bounded replay state rejects duplicate nonces | M04 retains durable envelopes and SQL idempotency; M19 retains Redis security/coordination and durable effects remain SQL-owned |
| M20 ownership boundary | Both proxy adapters depend only on `zbw-proxy-api`; platform transport is an async port and no Paper, Redis runtime, game, content, progression, statistics, replay or Atlas implementation is imported | M20 cross-server domain bridges and operational closure remain unimplemented |
## M20 Phase 3 cross-server flows

| Allocation | Materialized evidence | Boundary retained |
|---|---|---|
| M20 / ZBW-DEPLOY-002/004, ZBW-GAME-007 | `CrossServerIntent`, `OwnerHandoff`, `CrossServerWorkflow` and `CrossServerTransferResult` bind owner decisions to deterministic routing, 15-second reservations, single-use transfer and safe fallback/recovery; BungeeCord and Velocity dispatch the same prepared result | M10 remains the only matchmaking/selector policy owner; proxy code cannot create matches or decide eligibility |
| M20 / ZBW-ADDON-041..060 | PRIVATE_GAME intents carry bounded opaque roster references, private-game reference and settings version through owner-authorized reservation/transfer tests | Private-game lifecycle, roster rules and settings remain game/application-owned; no external party provider or M21 behavior |
| M20 / ZBW-ADDON-102..107, ZBW-REPLAY-001 | SPECTATE and REPLAY_SPECTATE intents carry target/session references only and use the same bounded transfer path | M10 spectator and M17 replay engines remain unchanged; no replay event or viewer state is copied |
| M20 / ZBW-ADDON-291..299 | REJOIN and PLAY_AGAIN intents prove owner authorization, atomic duplicate protection, expired-token recovery and configured lobby fallback | M08/M10 session/mode/map/party/cooldown policies remain authoritative; commands/GUI/placeholders are not claimed by this checkpoint |
| M20 / ZBW-ADDON-252..259 | MAP_SELECTION transports an owner-issued selection reference and capability request into route/reserve/transfer | Map eligibility/filter/selection policy remains owner-side |
| M20 / ZBW-ADDON-464..473, ZBW-ADDON-387 | `DomainVersionNotification` and bounded `VersionPropagationTracker` distribute only RESOURCE_SCARCITY and ITEM_ROTATION owner references plus strictly monotonic versions; duplicates/stale versions are rejected | M11 owns generator/scarcity/rotation state and M19 owns Redis coordination; proxy stores no content or schedule authority |
## M20 Phase 4 failure, operations and closure

| Allocation | Materialized evidence | Boundary retained |
|---|---|---|
| M20 / ZBW-DEPLOY-002..004, ZBW-READY-014 | `ProxyFailureSecurityPerformanceTest`, automatic heartbeat expiry, cached fail-closed coordination/backend/partition/drain gates and sanitized `ProxyDiagnostic` cover proxy/backend crash, acknowledgement loss, duplicates, stale registry/epochs, Redis loss and partition | M10 and each M11-M19 service remain authoritative; M19/backend admission must be shared by multiple proxies and SQL still owns durable effects |
