...
- **Current checkpoint:** PR #17 provides the original phases 1–4. M11.1 implements the bounded disabled-by-default scripting engine, transactional eight-section configuration activation, deterministic balance simulations, all twelve M11 mechanic families, atomic inventory purchase execution, cycle-free SQL preference/history/rotation recovery, M09 presentation bindings and exact Paper 1.21.1 projections. Mandatory publication workflows are defined, but GitHub currently rejects every PR #18 job before its first step because of the account billing/spending limit. `M11.1-MERGE-EXCEPTION-001` records local evidence only; it does not mark those workflows passed. M11 remains active and M12 remains blocked until the immutable commit passes every mandatory remote matrix.
...
- **Entry:** M04 outbox, M08 events and the M11 quoted-tender/match-resource contracts are stable. M12 remains blocked until PR #18 produces every mandatory M11.1 matrix and RC-087 is closed; this checkpoint does not start it.
- **Planned module:** Java-8-neutral `zbw-progression`, depending inward on `zbw-api`, `zbw-domain`, `zbw-application`, `zbw-storage-api`, `zbw-game` and `zbw-shop`. It is not materialized by this governance checkpoint. SQL adapters remain in `zbw-storage-sql`; presentation reuses M09 adapters.
