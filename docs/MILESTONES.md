...
- **Phase 1 status:** ACTIVE. Java-8-neutral `zbw-progression` is materialized with immutable typed progression, XP, level, prestige, persistent-currency, ledger, reward-registration and entitlement values; neutral repository ports; and M04-style idempotent projection/checkpoint/recovery contracts. No SQL/JDBC implementation, migration, reward delivery, GUI, command or Paper adapter is included. SQL adapters remain in `zbw-storage-sql`; future presentation reuses M09 adapters.
