...
- **Current checkpoint:** Phase 4 is implemented on the active M11 branch. Phases 1–3 provide shop/content, generators and team upgrades; Phase 4 adds the Java-8-neutral utility-item catalogue and synchronized action runtime with authorization, atomic match-resource/inventory boundaries, cooldowns, targets, idempotency, cleanup and bounded original mechanics for Pop-up Tower, Rush, Ultimate, BedSteal, Voidless, Sponge, local generator actions and Item Rotation. `zbw-scripting-engine`, final named-mode orchestration, feature presentation and primary Paper effect adapters remain unstarted. This checkpoint does not satisfy the M11 exit criteria.
