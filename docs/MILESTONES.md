...
- **Completion:** PR #17 provides the original phases 1–4. M11.1 completes the bounded disabled-by-default scripting engine, transactional eight-section configuration activation, deterministic balance simulations, all twelve M11 mechanic families, atomic inventory purchase execution, cycle-free SQL preference/history/rotation recovery, M09 presentation bindings and exact Paper 1.21.1 projections. The mandatory acceptance matrices and publication evidence are recorded in `docs/IMPLEMENTATION_M11_1.md`; M11 is complete without closing any retained later-milestone cell.
...
- **Entry:** M04 outbox, M08 events and the completed M11 quoted-tender/match-resource contracts are stable. M11.1 supplies every M11 exit artifact and RC-087 closure evidence, so M12 is the next planned milestone; this checkpoint does not start it.
