...
- **Current checkpoint:** Phase 3 is implemented on the active M11 branch. Phase 1 materialized `zbw-shop`, `zbw-content` and `zbw-scripting-api`; Phase 2 adds the neutral generator/resource runtime, M07 arena projection, M08 lifecycle consumption, bounded idempotent delivery, Generator Split and per-arena overrides; Phase 3 adds the neutral team-upgrade catalogue/state, atomic purchase boundary, queued traps, heal pool, dragon buff and deterministic forge runtime in `zbw-shop`. `zbw-scripting-engine`, utility items/actions, named-mode mechanics and feature presentation remain unstarted. This checkpoint does not satisfy the M11 exit criteria.
