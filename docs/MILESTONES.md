...
- **Current checkpoint:** Final-integration checkpoint is implemented on the active M11 branch. Phases 1–4 provide shop/content, generators, team upgrades and bounded utility/addon actions. The integration checkpoint consumes immutable M08 snapshots, publishes 25 additive M09 presentation actions and adds owner-thread-only Paper projections. `zbw-scripting-engine`, complete named-mode orchestration, concrete inventory rendering/configuration adapters and the full M11 acceptance matrices remain open, so M11 is not marked complete and M12 is not active.
