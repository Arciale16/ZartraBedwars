...
- **Current checkpoint:** PR #17 provides the original phases 1–4 and final integration. M11.1 Phase 1 now materializes the bounded disabled-by-default `zbw-scripting-engine`, the transactional eight-section M11 configuration activation boundary and deterministic mode-balance simulations. Complete named-mode orchestration, concrete inventory/Paper adapters and the full M11 acceptance matrices remain open, so M11 is not marked complete and M12 is not active.
