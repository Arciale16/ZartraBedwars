...
- **Current checkpoint:** Phase 2 is implemented on the active M11 branch. Phase 1 materialized `zbw-shop`, `zbw-content` and `zbw-scripting-api`; Phase 2 adds the neutral generator/resource runtime, M07 arena projection, M08 lifecycle consumption, bounded idempotent delivery, Generator Split and per-arena overrides in `zbw-shop`. `zbw-scripting-engine`, upgrades/traps/forge, items, named-mode mechanics and feature presentation remain unstarted. This checkpoint does not satisfy the M11 exit criteria.
