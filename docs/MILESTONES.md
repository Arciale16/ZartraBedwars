...
- **Current checkpoint:** Phase 1 shop/content foundations are implemented on the active M11 branch: `zbw-shop`, `zbw-content` and `zbw-scripting-api` are materialized, while `zbw-scripting-engine` and all later M11 mechanics/presentation remain unstarted. This checkpoint does not satisfy the M11 exit criteria.
