...
- **M20 Phase 3 status:** IN PROGRESS — owner-issued cross-server intents now cover remote queue/match handoff, private-game roster/settings references, spectate/replay-spectate, rejoin, play-again and map-selection transfer. `CrossServerWorkflow` performs only deterministic route/reserve/single-use transfer; Resource Scarcity and Item Rotation propagation carries only bounded monotonic owner versions. Both adapters dispatch identical prepared transfers. M10/M11/M12/M15/M17/M18/M19 ownership is unchanged; M21 providers, CloudNet, M22 compatibility and M20 closure remain unstarted.
