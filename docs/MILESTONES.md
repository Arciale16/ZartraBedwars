...
- **Current checkpoint:** PR #17 provides the original phases 1–4 and final integration. M11.1 Phase 1 materializes the bounded disabled-by-default `zbw-scripting-engine`, transactional eight-section M11 configuration activation and deterministic mode-balance simulations. M11.1 Phase 2 adds the neutral named-mode/addon mechanics boundary for the twelve M11 families using M08 lifecycle snapshots and M10 bindings. Concrete inventory/Paper adapters and final M11 acceptance matrices remain open, so M11 is not marked complete and M12 is not active.
...
- **Entry:** M04 outbox, M08 events and the M11 quoted-tender/match-resource contracts are stable, and M11 is recorded complete only after every M11 exit criterion has evidence. PR #17 is a merged implementation checkpoint but does not satisfy that final entry gate; M12 remains planned and inactive.
