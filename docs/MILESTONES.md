...
- **M20 Phase 1 status:** COMPLETE — `zbw-proxy-api` is materialized as a Java 8 neutral API; `zbw-bungeecord` and `zbw-velocity` remain planned only. ZBW-DEPLOY-002..004 receive immutable backend identity/epoch/capability, registry, heartbeat/capacity/health, deterministic routing, bounded reservation, single-use transfer-token outcome, protocol/degradation and sanitized diagnostic contracts. M04/M19 envelope, authentication, dedupe, lease and fencing ownership is reused rather than duplicated. No proxy adapter, Paper integration, Redis runtime change or M21 behavior exists.
- **M20 Phase 2 status:** COMPLETE — `zbw-bungeecord` (Java 8) and `zbw-velocity` (Java 21) are materialized over one Java 8 neutral runtime. ZBW-DEPLOY-002..004 and ZBW-READY-013/014 now have bounded epoch-aware registry/heartbeat/drain state, deterministic capacity routing with retry/fallback, atomic 15-second reservation/transfer consumption, HMAC-SHA-256 key rotation, version/environment/audience/deadline/nonce validation and identical nonblocking adapter semantics. M10 matchmaking and every M11-M19 durable/domain owner remain unchanged; cross-server feature flows remain later M20 work.
- **M20 Phase 3 status:** COMPLETE — owner-issued cross-server intents now cover remote queue/match handoff, private-game roster/settings references, spectate/replay-spectate, rejoin, play-again and map-selection transfer. `CrossServerWorkflow` performs only deterministic route/reserve/single-use transfer; Resource Scarcity and Item Rotation propagation carries only bounded monotonic owner versions. Both adapters dispatch identical prepared transfers. M10/M11/M12/M15/M17/M18/M19 ownership is unchanged; M21 providers, CloudNet and M22 compatibility remain outside M20.
- **M20 Phase 4 status:** COMPLETE — automatic stale-heartbeat expiry, fail-closed Redis/partition/backend/drain gates, bounded consumed-token retention, sanitized health diagnostics and the `PROXY_M20.md` runbook close operations. Deterministic tests cover crash/restart, acknowledgement loss, duplicate admission, stale epochs, security substitution/replay/clock/size/rate limits, rolling minor schemas, two-proxy shared admission and four-backend p95/p99 bounds. M20 is complete; M21 is next and is not started.
...
- **Phase 1 status:** COMPLETE - governance explicitly retains optional world-provider completion in M21; neutral provider SPIs plus native party core/SQL are materialized without vendor adapters, CloudNet or Paper implementation.
...
- **Phase 3 status:** COMPLETE - `zbw-cloudnet` provides optional discovery, secret-free metadata, bounded off-thread callbacks, fenced lifecycle actions, warm-pool scaling with hysteresis/cooldown, crash replacement and registry projections. Redis remains coordination, proxy remains routing, and CloudNet remains service lifecycle only.
- **Phase 4 certification status:** COMPLETE for the materialized nine-provider, native/external party, dual-anticheat and CloudNet lifecycle scope. Plugin Doctor reports PRESENT/ABSENT/INCOMPATIBLE/DUPLICATE; failure/race/restart/drain tests and the operations runbook pass.
- **Phase 5 status:** COMPLETE — the Java 8 `zbw-integration-world-providers` module materializes isolated WorldEdit, FAWE, WorldGuard, SlimeWorldManager and Multiverse adapters over the unchanged M06 SPI. Exact asynchronous compatibility probes, duplicate rejection, classloading isolation and native fallback are certified; no vendor dependency is bundled. M21 is complete, M22 is next and is not started.
- **Requirements:** DEPLOY-005, INT-001..003/005..009; ZBW-ADDON-226..235. `INT-005` optional WorldEdit/FAWE/WorldGuard/Slime/Multiverse adapters are complete; M06 remains owner of the native primary provider and M22 retains full runtime-matrix certification.
...
- **Phase 3 boundary:** the CloudNet artifact compiles only against neutral `zbw-api`, `zbw-redis-api` and `zbw-proxy-api` contracts. Operator-supplied bindings isolate CloudNet binaries; degraded Redis state pauses unsafe scaling, and no route, reservation or game rule enters the adapter. See `CLOUDNET_INTEGRATION_M21.md`.
...
- **Implementation status:** IMPLEMENTATION CLOSED. The Java 8 migration API and deterministic engine, lawful bounded layout adapter, compiled public-API-only SDK example and extensible operational Plugin Doctor probes are complete. The M23 implementation workstream is formally closed; ordered milestone advancement remains blocked by the active M22 external runtime/provider certification gate, and this closure does not begin M24.
...
- **Qualification status:** LOCAL IMPLEMENTATION COMPLETE; activation and final certification remain blocked. Static security, permission, privacy, dependency/provider-isolation, owner-thread, recovery-contract, reproducible-build and release-input gates are materialized in `build/m24-qualification.json` and `tools/validation/m24_qualification.py`. M22 remains implementation-complete but uncertified, all three normative runtime benchmarks and deployment recovery/security exercises remain pending, no compatibility support claim is allowed, and M23 contributes evidence only.
...
- **Pending activation gates:** (1) exact M22 runtime/client certification; (2) approved provider provenance, checksums, licence-text evidence and transitive lock/SBOM; (3) certified JDK 8/11/16/17/21 toolchains and lawful runtime fixtures; (4) the normative benchmark environment and workload fixtures; and (5) live security, privacy and recovery evidence. Until all five gates pass, `build/milestone-state.json` retains M22 as active and M24 as blocked with local qualification implemented.
