...
- **Preparation status:** PREPARATION ONLY; activation is blocked. M22 remains implementation-complete but uncertified, no compatibility support claim is allowed, and M24 cannot become active until the five prerequisite gates below are satisfied. M23 implementation is closed and contributes evidence only.
...
- **Pending activation gates:** (1) exact M22 runtime/client certification; (2) approved provider provenance, checksums, licence-text evidence and transitive lock/SBOM; (3) certified JDK 8/11/16/17/21 toolchains and lawful runtime fixtures; (4) the normative benchmark environment and workload fixtures; and (5) complete security, privacy, recovery and threat-model evidence inputs. Until all five gates pass, `build/milestone-state.json` retains M22 as active, retains the ordered M23 transition for validator compatibility, records M23 implementation closure separately and records M24 as blocked preparation.
