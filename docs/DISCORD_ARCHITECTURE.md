...
M02 supplies `DiscordIntegrationApi`, immutable `DiscordEventEnvelope`, `DiscordProvider` and `DiscordCapabilities` in `zbw-integration-discord-api`. M03 adds `DisabledDiscordProvider` plus the provider-neutral `SecretRef`, injected resolution, zeroization and redaction foundation. The disabled provider advertises no capability, performs no I/O, creates no thread and returns typed disabled/policy outcomes, so Minecraft startup and gameplay never require Discord. No webhook, external-bot transport, custom provider, outbox/retry engine or Discord SDK has been implemented; those remain assigned to M05/M16.
