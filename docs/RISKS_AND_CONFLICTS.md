...
- **Fact:** PR #17 left `zbw-scripting-engine`, complete named-mode orchestration, concrete inventory/configuration adapters and full M11 acceptance matrices open. M11.1 Phase 1 addresses the engine and neutral configuration/content foundations; Phase 2 addresses the twelve M11 mechanic families. Concrete inventory/Paper adapters and final acceptance matrices still block closure.
...
| ID | Classification | Finding / risk | Preserving resolution |
|---|---|---|---|
| RC-082 | Verified integrity defect — **RESOLVED 2026-07-15** | The initial setup preview model retained only the candidate fingerprint and compared it with itself during apply. That check could not prove that the session draft was still the base from which the candidate had been derived. | Store independent base and candidate fingerprints, create previews only from a base-bound candidate, reject apply after any intervening draft revision or fingerprint change, and retain the binding when a preview requests enable. Unit and exact Paper tests cover preview/apply and stale rejection; all M07 deterministic and quality validators pass with the corrected model. |
...
## L. M08/M09 allocation reconciliation

`RC-082` was already assigned by the merged M07 implementation to the independently verified setup-preview integrity defect above. Stable risk IDs are append-only, so the approved M08/M09 allocation finding uses the next available ID rather than overwriting or duplicating `RC-082`.

| ID | Classification | Finding / risk | Preserving resolution |
|---|---|---|---|
| RC-083 | Fact — **RESOLVED 2026-07-15** | M08 assigned six game requirements and 61 addon rows whose command/GUI/editor cells and `zbw-ui-paper` module references belong to M09, while Deposit, AntiDrop and AdventureMode referenced nonexistent `zbw-paper`. At the same time, M08 requires exact primary Paper E2E but `zbw-game` is platform-neutral and the planned graph did not record the later M08 composition edge into the already-materialized `zbw-paper-modern`. Without reconciliation, M08 either depends on M09, hides gameplay policy in presentation, or cannot meet its Paper exit criteria. | M08 owns Java-8 game/session/team/lobby policies and closed feature-specific Paper 1.21.1 projections; M09 owns all final production commands, GUIs, editors and confirmation infrastructure. `zbw-paper-modern` gains `zbw-game` only through an M08-qualified dependency; `zbw-game` depends on no M09-or-later module. All 61 addon rows retain explicit M08/M09 and M16/M20/M22 ownership, `ZBW-GAME-006` retains M21 NPC/hologram providers, and no feature is removed. Graph/order, traceability, catalogue, coverage, architecture and governance validators all passed before this conflict was marked resolved. |

## M. M08 implementation findings

| ID | Classification | Finding / risk | Preserving resolution |
|---|---|---|---|
| RC-084 | Verified dependency constraint — **RESOLVED BY ADR-0020** | The approved Paper API mirror is non-transitive, so direct linkage to player signatures requests unapproved Adventure artifacts. Treating compile-only success as runtime proof would also leave server-bound projections uncertified. | Keep all policy in Java-8 `zbw-game`; use a private allow-listed Java-21 reflection bridge only for closed Paper-owned values; add no dependency; retain 80%/70% Paper test-JVM coverage and require checksum-locked Paper 1.21.1 E2E for every excluded server-bound class. The E2E passes lifecycle, reconnect, exactly-once completion, restoration, boss-bar/listener cleanup and off-owner rejection; failure reopens RC-084. |

## N. M10/M11 and distributed-provider allocation reconciliation

| ID | Classification | Finding / risk | Preserving resolution |
|---|---|---|---|
| RC-085 | Fact — **RESOLVED 2026-07-17** | The original M10 milestone sentence required “all named modes” and every mode's complete statistics/shop/generator/upgrade/placeholder surface even though mode mechanics and balance packs depend on M11, statistics on M15, PlaceholderAPI on M16, proxy delivery on M20 and legacy compatibility on M22. Swappage's atomic catalogue rows and the spectator/play-again future `zbw-proxy-api` mapping made the overlap explicit. Read literally, M10 would either begin later milestones or falsely mark framework registrations complete. | M10 owns Java-8-neutral mode registration/selection and deferred bindings, local selectors/matchmaking/reservations, spectator, Compass and Team Selector behavior. M11 owns all named-mode gameplay mechanics, including the completion portions of Swappage; M15/M16/M20/M22 retain their listed cells. M10 adds no module and no dependency on a future proxy module; it emits typed local routing intent for M20. Traceability, architecture, catalogue and milestone exit language now state the split without removing any requirement. RC-085 remains resolved only while deterministic governance, 49/473 addon, 672-requirement and 6,966-atomic-item coverage gates pass. |

## O. M11 shop/platform and later-consumer allocation reconciliation

| ID | Classification | Finding / risk | Preserving resolution |
|---|---|---|---|
| RC-086 | Fact — **RESOLVED 2026-07-17** | M11 was assigned complete SHOP-001..007 and 132 addon rows even though SHOP-003/004 depended on M12 persistent currency and M21 Vault, addon rows embedded M15 statistics and M16 placeholders, ZBW-ADDON-387 embedded M19/M20 cross-node synchronization, concrete shopkeeper/hologram providers belonged to M21, and legacy/fallback acceptance belonged to M22. The architecture named four M11 modules that were absent from the machine graph, while the unsplit exit language could falsely close later work. | M11 owns quoted match-resource transactions, shop/generator/upgrade/item mechanics, named-mode mechanics, original balance packs, the declarative sandbox, feature-specific reuse of M09 presentation and primary Paper 1.21.1 translation. M12/M15/M16/M19/M20/M21/M22 retain their explicit consumers/providers. The planned graph allocates four Java-8 M11 modules and milestone-qualified adapter/composition dependencies; Phase 1 materializes only `zbw-shop`, `zbw-content` and `zbw-scripting-api`, with the engine and later mechanics still deferred. Swappage remains M10 registration, M11 mechanics, M15 statistics, M16 placeholders and M22 compatibility. The conflict remains resolved only while module-order, traceability, addon, dashboard, 672-requirement, 473-addon and 6,966-atomic-item validators pass. No requirement or later ownership is removed. |

## P. M11 Phase 1 implementation findings

| ID | Classification | Finding / risk | Preserving resolution |
|---|---|---|---|
| RC-087 | Verified reactor constraint — **OPEN; non-blocking for Phase 1** | Activating the planned `zbw-storage-sql -> zbw-shop` adapter edge before an adapter exists closes the current test-only reactor path `zbw-arena -> zbw-storage-sql -> zbw-shop -> zbw-arena`. Keeping the edge now would make the build cyclic without delivering persistence behavior. | Phase 1 publishes only neutral asynchronous preference/history/rotation ports and does not claim a SQL adapter. A later M11 persistence phase must first extract or invert the arena SQL test-fixture dependency, then implement and contract-test the adapter before activating the planned edge. No persistence behavior or later milestone is claimed by this checkpoint. |
