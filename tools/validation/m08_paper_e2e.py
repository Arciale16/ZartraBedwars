#!/usr/bin/env python3
"""Run the test-artifact-only M08 game/projection scenario on locked Paper."""
...
HARNESS_CLASS = "io/zartra/bedwars/paper/m08/M08PaperCertificationPlugin"
...
def build_harness(harness_classes: Path, destination: Path) -> None:
    """Package only the M08 test certification entry point."""
    ...
    if not selected:
        raise ValueError("compiled M08 Paper certification class is missing")
    with zipfile.ZipFile(destination, "w", zipfile.ZIP_DEFLATED) as bundle:
        bundle.writestr(
            "plugin.yml",
            "name: ZartraBedWarsM08Certification\n"
            "version: '0.1.0-SNAPSHOT'\n"
            "main: io.zartra.bedwars.paper.m08.M08PaperCertificationPlugin\n"
            "api-version: '1.21'\n"
            "load: POSTWORLD\n"
...
def write_properties(directory: Path) -> None:
    ...
    (directory / "server.properties").write_text(
        ...
        "level-name=m08_primary\nmotd=ZartraBedWars M08 certification\n",
        encoding="utf-8",
        newline="\n",
    )
...
def validate_evidence(evidence: dict[str, object], server_sha256: str,
    ...
    """Enforce every exact-runtime M08 assertion."""
    expected = {
        "runtime": "Paper 1.21.1 build 133",
        "server_sha256": server_sha256,
        "waiting_through_reset": True,
        "reconnect_recovery": True,
        "exactly_once_completion": True,
        "player_state_restoration": True,
        "bossbar_create_update_remove": True,
        "event_runtime_register_unregister": True,
        "off_owner_mutation_rejected": True,
    ...
    if failures:
        raise ValueError("M08 Paper certification failed:\n- " + "\n- ".join(failures))
...
def run(arguments: argparse.Namespace) -> int:
    ...
    harness = plugins / "ZartraBedWarsM08Certification.jar"
    ...
    (run_directory / "m08-paper-console.log").write_text(
        output, encoding="utf-8", newline="\n"
    )
    source = plugins / "ZartraBedWarsM08Certification" / "m08-primary-certification.json"
    require_file(source, "M08 runtime evidence")
    ...
    evidence["certification_runner"] = "tools/validation/m08_paper_e2e.py"
    ...
    print("M08 Paper E2E PASS: lifecycle/reconnect/exactly-once/restoration; "
          "boss-bar/runtime cleanup; owner-thread rejection")
    return 0
...
def main() -> int:
    ...
    parser.add_argument("--run-directory", type=Path,
                        default=ROOT / "target/m08-paper-e2e")
    parser.add_argument("--evidence-output", type=Path,
                        default=ROOT / "build/evidence/m08-paper-primary.json")
