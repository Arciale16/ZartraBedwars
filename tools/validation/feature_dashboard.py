...
def row(feature: str, identifier: str, category: str, planned: str,
    ...
    return {
        ...
        "Tests": "M13 model/runtime/recovery/presentation/Paper evidence" if m13 else ("M12 unit/integration/recovery/Paper certification evidence" if m12 else ("M11 unit/integration/security/Paper certification evidence" if m11 else ("M10 unit/quality/Paper evidence" if m10 else ("M09 unit/parity/Paper E2E" if m09 else "Milestone evidence")))),
        "Documentation": "M13 Phase 1-3 implementation/API/command/GUI evidence" if m13 else ("M12 phase implementation/API and closure evidence" if m12 else ("M11 and M11.1 implementation/API evidence" if m11 else ("M10 guides and inventories" if m10 else ("M09 framework/inventories" if m09 else "PRD + traceability")))),
...
def render() -> str:
    ...
    lines.extend([
        ...
        "M00–M13 and hardening M08.1 are complete; M14 is the next active milestone.",
        ...
        "contracts, deterministic runtime, SQL persistence and M09/Paper presentation; later owners remain unclaimed.",
        "", "## Feature rows", "",
    ])
    columns = list(rows[0])
