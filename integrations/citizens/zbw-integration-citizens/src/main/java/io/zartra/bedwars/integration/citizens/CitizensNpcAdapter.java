...
/** Optional Citizens presentation adapter; definitions remain provider-neutral. */
public final class CitizensNpcAdapter implements NpcProvider {
    private final Gateway gateway;
    private final OptionalProviderLifecycle lifecycle;

    /** @param gateway Citizens presentation boundary @param probe availability
     * @param timeSource observation clock */
    public CitizensNpcAdapter(final Gateway gateway,
                              final OptionalProviderLifecycle.Probe probe,
                              final TimeSource timeSource) {
        this.gateway = Objects.requireNonNull(gateway, "gateway");
        lifecycle = new OptionalProviderLifecycle(ProviderId.of("zartra", "citizens"),
                SemanticVersion.parse("2.0.39"),
                CapabilitySet.of(Collections.singletonList(
                        CapabilityId.of("zartra", "npc"))),
                timeSource, "provider.citizens", probe);
    }
    ...
    private static <T> Result<T> failure() {
        return Result.failure(ApiError.of(
                DefinitionId.of("zartra", "provider/citizens-unavailable"),
                "provider.citizens_unavailable", ApiError.RetryDisposition.RETRYABLE));
    }

    /** Narrow runtime binding to the operator-installed Citizens API. */
