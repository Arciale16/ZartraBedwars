...
/** Optional DecentHolograms presentation adapter with bounded provider-neutral definitions. */
public final class DecentHologramsAdapter implements HologramProvider {
    private final Gateway gateway;
    private final OptionalProviderLifecycle lifecycle;

    /** @param gateway hologram presentation boundary @param probe availability
     * @param timeSource observation clock */
    public DecentHologramsAdapter(final Gateway gateway,
                                  final OptionalProviderLifecycle.Probe probe,
                                  final TimeSource timeSource) {
        this.gateway = Objects.requireNonNull(gateway, "gateway");
        lifecycle = new OptionalProviderLifecycle(
                ProviderId.of("zartra", "decentholograms"),
                SemanticVersion.parse("2.9.6"),
                CapabilitySet.of(Collections.singletonList(
                        CapabilityId.of("zartra", "hologram"))),
                timeSource, "provider.decentholograms", probe);
    }
    ...
    private static <T> Result<T> failure() {
        return Result.failure(ApiError.of(
                DefinitionId.of("zartra", "provider/decentholograms-unavailable"),
                "provider.decentholograms_unavailable",
                ApiError.RetryDisposition.RETRYABLE));
    }

    /** Narrow runtime binding to the operator-installed DecentHolograms API. */
